...
class TrafficModel(Model, ABC):
    ...
    def flows_on_shortest(
        ...
        return flows_on_shortest_gt(
            ...
            maybe_create_and_get_times_ep(self.graph, np.asarray(times, dtype=np.float64)),
            return_distance_mat,
        )
...
class TwostageModel(Model):
    ...
    def distance_mat(self, times) -> np.ndarray:
        ...
        return distance_mat_gt(
            ...
            sources,
            targets,
            maybe_create_and_get_times_ep(self.traffic_model.graph, np.asarray(times, dtype=np.float64)),
        )
